...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        run = start_run(
            ...
            name='refresh_macro_data',
            steps=[
                {'label': '指標取得', 'command': 'sync_all_indicators'},
                {'label': '景気判定', 'command': 'compute_current_regime'},
                {'label': 'World State 計算', 'command': 'compute_world_state'},
                {'label': '価格取得', 'command': 'sync_all_price_histories'},
                {'label': '直近日次価格取得', 'command': 'sync_daily_prices'},
            ],
        )
        if not get_api_key():
            _save_status(
            ...
            finish_run(
                ...
                summary={'message': 'FRED_API_KEY が未設定です。'},
        ...
        try:
            ...
            finish_run(
                run,
                status=WorldModelRun.Status.FAILED,
                summary={'message': '指標取得処理が途中で失敗しました。'},
                error=str(exc),
            )
            raise
        ...
        if ok == 0:
            ...
            finish_run(
                run,
                status=WorldModelRun.Status.FAILED,
                summary={'message': '1 件も取得できなかったため失敗しました。'},
            )
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f'World State 計算に失敗: {exc}')
            )
        ...
        try:
            ...
            finish_run(
                run,
                status=WorldModelRun.Status.PARTIAL,
                summary={'message': '価格データ更新に失敗しました。'},
                error=str(exc),
            )
        ...
        try:
            call_command('sync_daily_prices', days=120)
        except Exception as exc:
            extra_failed.append({'phase': 'daily_price_sync', 'error': str(exc)})
            self.stdout.write(
                self.style.WARNING(f'日次価格更新に失敗: {exc}')
            )
        _save_status(
            result=result,
            message='日次更新を実行しました。',
        ...
        finish_run(
            run,
            status=WorldModelRun.Status.PARTIAL if extra_failed else WorldModelRun.Status.SUCCESS,
            summary={
                'message': '日次更新を実行しました。',
                'indicator_success_count': ok,
                'indicator_failed_count': ng,
                'price_success_count': price_ok,
                'price_failed_count': price_ng,
            },
        )
