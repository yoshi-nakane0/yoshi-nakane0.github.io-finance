...
class ExplanationDecisionEngineTests(SimpleTestCase):
    ...
    def test_trade_confidence_uses_weighted_components_instead_of_simple_minimum(self):
        ...
        self.assertEqual(decision.confidence_components['validation_level'], 'low')
...
class ExplanationBasecalcAdapterTests(SimpleTestCase):
    ...
    def test_load_basecalc_signal_prefers_output_contract_capped_confidence(self):
        snapshot = {
            'generated_at': '2026-06-23T09:39:11+00:00',
            'world_model': {
                'direction': 'up',
    ...
    def test_load_basecalc_signal_keeps_zero_output_contract_confidence(self):
        ...
        self.assertEqual(signal.confidence_score, 0)
        self.assertEqual(signal.confidence_grade, 'D')

    def test_load_basecalc_signal_falls_back_to_basecalc_signal_reason_separation(self):
        snapshot = {
            'generated_at': '2026-06-23T09:39:11+00:00',
            'world_model': {
                'direction': 'up',
        ...
        self.assertEqual(signal.hard_block_reasons, [])
        self.assertEqual(signal.soft_warning_reasons, ['米国3指数確認が不足'])
        self.assertEqual(signal.validation_warnings, ['局面別検証不足のため信頼度を限定'])
        self.assertEqual(signal.confidence_cap_reason, '局面別検証不足のため信頼度を限定')
        self.assertEqual(signal.display_status, 'candidate_limited')
        self.assertIn('米国3指数確認が不足', signal.warnings)
...
class ExplanationViewCompositionTests(SimpleTestCase):
    ...
    def test_explanation_template_candidate_trade_conditions_name_watch_only_state(self):
        ...
        self.assertIn('売買可否', trade_condition_html)
        self.assertIn('監視のみ', trade_condition_html)
        self.assertNotIn('待機 / 条件待ち', trade_condition_html)
    ...
    def test_beginner_decision_names_watch_only_availability_without_wait_label(self):
        snapshot = self._snapshot()
        snapshot.trade_decision.update({
            'selected_side': 'long',
            'decision_type': 'trend_follow',
            ...
            'current_price': 72430,
            'entry_price': 72430,
            'target_1': {'label': 'T1', 'price': 73800, 'probability': 0.42},
            'stop_price': 71600,
            'invalidation_price': 71600,
            'reward_risk': 1.65,
            'confidence_score': 45,
        ...
        self.assertEqual(beginner['label'], '監視のみ')
        self.assertEqual(beginner['trade_availability_display'], '監視のみ')
        self.assertNotEqual(beginner['trade_availability_display'], '待機 / 条件待ち')
    ...
    def test_beginner_decision_shows_limited_candidate_plan(self):
        ...
        self.assertEqual(beginner['candidate_status'], 'candidate_limited')
    ...
    def test_beginner_decision_trusts_confirmed_decision_status_over_score_gate_recheck(self):
        snapshot = self._snapshot()
        snapshot.trade_decision.update({
            'selected_side': 'long',
            'decision_type': 'trend_follow',
            ...
            'short_score': 30,
            'no_trade_score': 58,
            'reasons': ['TradeDecisionで通常候補まで確認済み'],
        })

        context = snapshot_to_view(snapshot)
        beginner = context['beginner_decision']

        self.assertEqual(beginner['label'], '買い候補')
        self.assertTrue(beginner['candidate_visible'])
        self.assertTrue(beginner['execution_allowed'])
        self.assertTrue(beginner['position_allowed'])
        self.assertEqual(beginner['trade_availability_display'], '通常候補')
        self.assertEqual(beginner['position_size_display'], '通常サイズ')
        self.assertNotIn('スコア不足', beginner['wait_reasons'])
        self.assertNotIn('no_tradeより弱い', beginner['wait_reasons'])
    ...
    def test_beginner_decision_exposes_confidence_component_rows(self):
        ...
        self.assertIn({'label': '検証状態', 'value': '検証少'}, rows)
        self.assertIn({'label': 'target品質', 'value': '80点'}, rows)
        self.assertIn({'label': 'R/R', 'value': '1.65'}, rows)
        ...
        self.assertIn({'label': '停止減点', 'value': '-30点'}, rows)
    ...
    def test_template_renders_confidence_component_rows(self):
        ...
        self.assertIn('1.65', html)
        self.assertIn('停止減点', html)
        self.assertIn('-30点', html)
...
class ExplanationIntegrityCommandTests(SimpleTestCase):
    def test_integrity_rejects_trade_decision_missing_extended_contract_fields(self):
        required_fields = (
            'decision_status',
            'entry_permission',
            'validation_level',
        ...
        for missing_key in required_fields:
            with self.subTest(missing_key=missing_key), TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                ...
                trade_decision.pop(missing_key)
                latest_payload = {
                ...
                with self.assertRaisesMessage(CommandError, f'trade_decision must include {missing_key}'):
                    call_command(
    ...
    def test_integrity_rejects_trade_decision_extended_contract_field_type_mismatch(self):
        invalid_cases = (
            ('hard_block_reasons', '現在値不一致', 'trade_decision hard_block_reasons must be a list'),
            ('soft_warning_reasons', '検証不足', 'trade_decision soft_warning_reasons must be a list'),
            ('confidence_components', [], 'trade_decision confidence_components must be a dict'),
        )
        for key, invalid_value, expected_message in invalid_cases:
            with self.subTest(key=key), TemporaryDirectory() as tmpdir:
                ...
                trade_decision[key] = invalid_value
                latest_payload = {
                    'snapshot_key': 'snapshot-1',
                    'schema': 'explanation_snapshot_v1',
                    'generated_at': '2026-06-25T01:16:00+00:00',
    ...
    def test_integrity_rejects_trade_decision_unknown_validation_level(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'trade_decision validation_level is not allowed: confirmed'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_trade_decision_unknown_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'trade_decision entry_permission is not allowed: paper_entry'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_limited_candidate_without_trade_side(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_limited selected_side must be long or short'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_missing_separated_scores(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle must include decision_confidence_score'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_score_mismatching_validation_readiness(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            latest_payload = {
                ...
                'score_bundle': {
                    ...
                    'decision_confidence_label': '判定停止',
                    'validation_readiness_score': 69,
                    'validation_readiness_label': '検証参考',
                    'system_quality_components': [
                        {
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle score must match validation_readiness_score'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_decision_confidence_score_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle decision_confidence_score must match trade_decision'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_decision_confidence_label_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle decision_confidence_label must match trade_decision'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_system_quality_label_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle system_quality_label must match system_quality_score'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_validation_readiness_label_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            latest_payload = {
                ...
                'score_bundle': {
                    ...
                    'decision_confidence_label': '限定候補',
                    'validation_readiness_score': 69,
                    'validation_readiness_label': '実績確認済み',
                    'system_quality_components': [
                        {
                            'label': '判定契約',
                            'score': 20,
            ...
            with self.assertRaisesMessage(CommandError, 'score_bundle validation_readiness_label must match validation_readiness_score'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_score_bundle_label_mismatching_validation_readiness_label(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
    ...
    def test_integrity_rejects_stale_score_bundle_decision_contract_row_in_history(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
    ...
    def test_integrity_rejects_legacy_basecalc_display_status_name(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
    ...
    def test_integrity_rejects_limited_candidate_with_no_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_limited entry_permission must be limited_entry'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_limited_candidate_with_full_position_size(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_limited position_size_pct must be 25 or 50'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_candidate_with_hard_block_reasons(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_limited hard_block_reasons must be empty'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_confirmed_candidate_without_full_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_confirmed entry_permission must be full_entry'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_confirmed_candidate_without_full_position_size(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_confirmed position_size_pct must be 100'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_confirmed_candidate_without_high_validation_level(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_confirmed validation_level must be high'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_confirmed_candidate_with_soft_warnings(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'candidate_confirmed soft_warning_reasons must be empty'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_watch_only_without_trade_side(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'watch_only selected_side must be long or short'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_watch_only_with_hard_block_reasons(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            latest_payload = {
                ...
                'trade_decision': {
                    ...
                    'entry_permission': 'watch_only',
                    'validation_level': 'low',
                    'hard_block_reasons': ['target/stop不整合'],
                    'soft_warning_reasons': ['R/R不足'],
                    'confidence_components': {},
                    'position_size_pct': 0,
                    'confidence_score': 45,
            ...
            with self.assertRaisesMessage(CommandError, 'watch_only hard_block_reasons must be empty'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_watch_only_with_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'watch_only entry_permission must be watch_only'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_watch_only_with_position_size(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'watch_only position_size_pct must be 0'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_blocked_decision_with_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'blocked entry_permission must be no_entry'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_blocked_decision_with_position_size(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'blocked position_size_pct must be 0'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_blocked_decision_without_hard_block_reasons(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'blocked hard_block_reasons must not be empty'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_wait_decision_with_entry_permission(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesMessage(CommandError, 'wait entry_permission must be no_entry'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_wait_decision_with_position_size(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
    ...
    def test_integrity_rejects_wait_decision_with_hard_block_reasons(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'wait hard_block_reasons must be empty'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_error_basecalc_contract_with_nonzero_confidence(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc error contract confidence_score must be 0'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_error_basecalc_contract_with_non_d_confidence_label(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc error contract confidence_label must be D'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_error_basecalc_contract_with_candidate_display_status(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc error contract display_status must be blocked'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_basecalc_raw_world_model_display_status_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc world_model display_status must match output_contract'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_basecalc_raw_world_model_explanation_allowed_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc world_model explanation_allowed must match output_contract'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
    ...
    def test_integrity_rejects_basecalc_top_level_explanation_allowed_mismatch(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            latest = root / 'latest_snapshot.json'
            history = root / 'snapshot_history.json'
            ...
            with self.assertRaisesMessage(CommandError, 'basecalc explanation_allowed must match output_contract'):
                call_command(
                    'check_explanation_integrity',
                    latest=str(latest),
                    history=str(history),
...
class ExplanationSnapshotFactoryPersistenceTests(TestCase):
    ...
    def test_build_snapshot_persists_basecalc_contract_display_fields(self):
        from .services.contracts import BasecalcSignal, MacroSignal
        from .services.factory import build_explanation_snapshot
        ...
        self.assertEqual(saved_basecalc['display_status'], 'blocked')
        self.assertEqual(saved_basecalc['explanation_allowed'], 'blocked')
